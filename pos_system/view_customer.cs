...
namespace pos_system
{
    public partial class view_customer : Form
    {
        ...
        {
            customer_key = Convert.ToInt32(guna2DataGridView1.SelectedRows[0].Cells[0].Value.ToString());
        ...
        private void bunifuThinButton21_Click_1(object sender, EventArgs e)
        {
            if (customer_key == 0)
            {
                Mbox.mShow("Select a Customer");
            }
            else
            {
                try
                {
                    SqlCommand cmd = new SqlCommand("delete from customer where id= '" + customer_key + "'", conn);
                    cmd.ExecuteNonQuery();
                    Mbox.mShow("Customer Deleted Successfully");
                    conn.Close();
                    fetch_customers();
                }
                catch (Exception ex)
        ...
        private void bunifuThinButton22_Click_2(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    SqlCommand cmd = new SqlCommand("update customer set name= '" + textBox1.Text + "', address= '" + textBox2.Text + "', phone= '" + textBox3.Text + "'where id= '" + customer_key + "'", conn);
                    cmd.ExecuteNonQuery();
                    Mbox.mShow("Customer Updated Successfully");
                    conn.Close();
                    fetch_customers();
                }
                catch (Exception ex)
